...
class InfiniteView extends Backbone.View
{
    initialize(options)
    {
        ...
        new AddView({
            el: this.$el.closest('.wizhi-infinite-container').children('div.wizhi-infinite-add-field-container'),
            parent: this
        });
    ...
    sort()
    {
        this.$el.sortable({
            ...
            placeholder: 'wizhi-ui-state-highlight',
            handle: '.wizhi-infinite-order',
            update: function()
            {
    ...
    getFirstRow()
    {
        let row = this.$el.find('tr.wizhi-infinite-row').first().clone(),
            rowView = new RowView({
                el: row,
                parent: this
            });
    ...
    updateCount()
    {
        this.count = this.$el.children('tr.wizhi-infinite-row').length;
    }
    ...
    rename()
    {
        let rows = this.$el.children('tr.wizhi-infinite-row');
        ...
        _.each(rows, (row, index) =>
        {
            ...
            // Get row fields.
            let fields = row.find('td.wizhi-infinite-inner>table>tbody').first().children(), // tr.wizhi-field-container element
                order = row.children('td.wizhi-infinite-order').children('span');
            ...
            _.each(fields, field =>
            {
                ...
                let input = field.find('input, textarea, select'),
                    label = field.find('th.wizhi-label>label'),
                    collectionField = field.find('.wizhi-collection-wrapper'); // Check if there is a collection field.
                ...
                else
                {
                    ...
                    let items = collectionField.find('ul.wizhi-collection-list input');
    ...
    renameCollectionField(field, index)
    {
        let regex = new RegExp("([0-9]+)\]"),
            ...
            template = field.find('script#wizhi-collection-item-template'),
