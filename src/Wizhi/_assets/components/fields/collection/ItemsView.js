...
class ItemsView extends Backbone.View
{
    ...
    {
        return {
            'click button#wizhi-collection-add': 'add',
            'click button#wizhi-collection-remove': 'removeSelectedItems'
        };
    }
    ...
    insertItem(attachment)
    {
        ...
        this.$el.find('ul.wizhi-collection-list').append(itemView.render().el);
    }
    ...
    getAttachmentThumbnail(attachment)
    {
        ...
        if('image' === type)
        {
            // Check if the _wizhi_media size is available.
            let sizes = attachment.get('sizes');

            if (undefined !== sizes._wizhi_media)
            {
                url = sizes._wizhi_media.url;
            }
            else
            {
    ...
    toggleRemoveButton(items)
    {
        ...
        if (length)
        {
            ...
            this.$el.find('button#wizhi-collection-remove').addClass('show');
        }
        else
        {
            // Hide the main remove button.
            this.$el.find('button#wizhi-collection-remove').removeClass('show');
        }
    }
    ...
    toggleCollectionContainer(collection)
    {
        let length = collection.length,
            addButton = this.$el.find('button#wizhi-collection-add'),
            container = this.$el.find('div.wizhi-collection-container');
    ...
    sort()
    {
        this.$el.find('ul.wizhi-collection-list').sortable({
            ...
            placeholder : 'wizhi-collection-ui-state-highlight',
            handle : '.wizhi-collection__item'
        });
    }
}
