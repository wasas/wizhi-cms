...
use Wizhi\View\Engines\TwigEngine;
use Wizhi\View\Extensions\WizhiTwigExtension;
...
class ViewServiceProvider extends ServiceProvider {
	...
	protected function registerTwigEnvironment() {
		...
		$container[ 'twig' ]->addExtension( new WizhiTwigExtension( $container ) );
	}
	...
	public function boot() {
		...
		$blade->directive( 'query', function ( $expression ) {
			return '<?php $_wizhiQuery = (is_array(' . $expression . ')) ? new WP_Query(' . $expression . ') : ' . $expression . '; if($_wizhiQuery->have_posts()) { while($_wizhiQuery->have_posts()) { $_wizhiQuery->the_post(); ?>';
		} );
